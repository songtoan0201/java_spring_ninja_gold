...
public class HomeController {
	...
	public String secretCode(@RequestParam String which_form, HttpSession session, HttpServletRequest request) {
		...
		if(request.getParameter("which_form").equals("farm")) {
			...
			strArr.add(String.format("<option style='color:green'>Earn %d from farming (%s)</option>", session.getAttribute("farm_gold"), session.getAttribute("dateFormat") ));
			...
			strArr.add(String.format("<option style='color:green'>Earn %d from farming (%s)</option>", session.getAttribute("cave_gold"), session.getAttribute("dateFormat") ));
			session.setAttribute("activities", strArr);		
			...
			strArr.add(String.format("<option style='color:green'>Earn %d from farming (%s)</option>", session.getAttribute("house_gold"), session.getAttribute("dateFormat") ));
			session.setAttribute("activities", strArr);		
			...
			if ((int)session.getAttribute("total_gold") > 0) {
				...
				if ((int)session.getAttribute("casino_gold") < 0) {
					strArr.add(String.format("<option style='color:red'>Lose %d from farming (%s)</option>", -(int)session.getAttribute("casino_gold"), session.getAttribute("dateFormat") ));
					session.setAttribute("activities", strArr);		
				}
				else {
					strArr.add(String.format("<option style='color:green'>Earn %d from farming (%s)</option>", session.getAttribute("casino_gold"), session.getAttribute("dateFormat") ));
					session.setAttribute("activities", strArr);	
				}
			} else {
				strArr.add(String.format("<option style='color:red'>You have no money to enter casino (%s)</option>", session.getAttribute("dateFormat") ));
				session.setAttribute("activities", strArr);	
			}
